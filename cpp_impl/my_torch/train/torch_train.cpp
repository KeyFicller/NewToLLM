...
int64_t estimate_num_batches(const std::vector<int64_t> &token_ids,
                             int64_t max_length, int64_t stride,
                             int64_t batch_size) {
...
int64_t estimate_num_samples(const std::vector<int64_t> &token_ids,
                             int64_t max_length, int64_t stride) {
  if (static_cast<int64_t>(token_ids.size()) <= max_length) {
    return 0;
  }
  return ((static_cast<int64_t>(token_ids.size()) - max_length - 1) / stride) +
         1;
}

void generate_and_print_sample(ToyModel &model, tokenizers::Tiktoken &tokenizer,
                               torch::Device device,
  ...
  auto out_advanced = generate_text_advanced(model, idx, 50, context_length,
                                             1.4, 25, c10::nullopt);
  auto flat_advanced = out_advanced.squeeze(0).to(torch::kLong).to(torch::kCPU);
  std::vector<int64_t> out_ids_advanced;
  out_ids_advanced.reserve(static_cast<size_t>(flat_advanced.size(0)));
  for (int64_t i = 0; i < flat_advanced.size(0); ++i) {
    out_ids_advanced.push_back(flat_advanced[i].item<int64_t>());
  }
  auto decoded_advanced =
      normalize_for_log(decode_ids(tokenizer, out_ids_advanced));
  std::cout << "[train] Generated text advanced: " << decoded_advanced << '\n';
...
void train_torch() {
  ...
  if (!pre_ids.empty()) {
    ...
    torch::NoGradGuard no_grad;
    auto pre_out =
        generate_text_simple(model, pre_input, 10, cfg.context_length);
  ...
  const auto train_batches = estimate_num_batches(
      train_ids, cfg.context_length, cfg.context_length, batch_size);
  const auto val_batches = estimate_num_batches(val_ids, cfg.context_length,
                                                cfg.context_length, batch_size);
  ...
  std::cout << "[train] Train loader samples: " << train_samples
            << ", batches: " << train_batches << ", batch_size: " << batch_size
            << '\n';
  std::cout << "[train] Validation loader samples: " << val_samples
            << ", batches: " << val_batches << ", batch_size: " << batch_size
            << '\n';
  ...
  torch::optim::AdamW optimizer(
      model->parameters(),
      torch::optim::AdamWOptions(0.0004).weight_decay(0.1));
  ...
  for (int64_t epoch = 0; epoch < num_epochs; ++epoch) {
    ...
    for (auto &batch : *train_loader) {
      ...
      auto loss = torch::nn::functional::cross_entropy(logits.flatten(0, 1),
                                                       y.flatten());
      loss.backward();
      optimizer.step();
      ...
      if (global_step % eval_freq == 0) {
        ...
        auto val_eval_loader = create_eval_data_loader(
            val_ids, batch_size, cfg.context_length, cfg.context_length);
        const auto [train_loss, val_loss] = evaluate_model(
            model, *train_eval_loader, *val_eval_loader, device, eval_iter);
